...
const initMySQL = async () => {
    ...
    const connection = mysql.createConnection ({
        host: process.env.DB_HOST, // หรือใส่เป็น localhost ก็ได้
...
app.get("/users", async (req,res) => {
    try {
        connection.query("SELECT * FROM stc_base", async (err, results, fields) => {
           ...
           const request = 'ID :'+results[0].id+'                User :'+results[0].fullname+'                   Date :'+results[0].date+'                    Time :'+results[0].time
           await line(request);
...
app.get("/user/single/:id", async (req,res) => {
    ...
    try {
        const results = await new Promise((resolve, reject) => {
            connection.query("SELECT * FROM stc_base WHERE id = ?", [id], (err, results, fields) => {
                if (err) {
                    console.log(err);
                    reject(err);
                } else {
        ...
        const request = 'คุณ :' +results[0].fullname+ '     วันที่ :' +results[0].date
        await line(request);
        res.status(200).json(results);
    } catch (err) {
        console.log(err);
...
app.patch("/update/:id", async (req, res) => {
    ...
    try {
        connection.query("UPDATE stc_base SET password = ? WHERE id =?",[newFullname,id], (err, results, fields) => {
           if(err) {
            console.log(err);
            return res.status(400).send();
           } 
...
app.delete("/delete/:id", async (req, res) => {
    ...
    try {
        connection.query("DELETE FROM stc_base WHERE id = ?",[id], (err, results, fields) => {
           if(err) {
            console.log(err);
            return res.status(400).send();
           } 
